...
public class Face {
	...
	public static void showDialog(final Context ctx){
		...
		final Dialog dialog = new AlertDialog.Builder(ctx).setTitle("选择表情")
				.setView(gridView)
		...
		gridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				dialog.dismiss();
				Map.Entry entry=(Map.Entry)Face.getfacesList(ctx ).get(position).entrySet().iterator().next();
				int fid=(Integer)entry.getValue();
				String facename=Face.findFaceNameByFaceId(ctx, fid);
				if(lis!=null){
	...
	private static GridView createGridView(Context ctx) {
		...
		List<Map<String, Integer>> listItems =Face.getfacesList(ctx);
		SimpleAdapter simpleAdapter = new SimpleAdapter(ctx, listItems,
				R.layout.view_item_face,

 ...
 */
public class FileSelectedAdapter extends BaseAdapter{
    private List<FileUpload> list;
    private Context context;

    public FileSelectedAdapter(List<FileUpload> list, Context context) {
        this.list = list;
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if(position == list.size()){
            ...
            if(ValidationUtils.validationString(list.get(position).getNewfilename())){
                nameTxt.setText(list.get(position).getFilename());
                nameTxt.setVisibility(View.VISIBLE);
                musicCoverImg.setImageBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.file_default));
            }
        }
        return convertView;
    }
